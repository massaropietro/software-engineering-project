...
def test_project_validator():
    ...
    with pytest.raises(ValidationError, match="You must provide either a zip file or a repository URL."):
        validator.validate(project_invalid)

@pytest.mark.django_db
def test_trigger_filesystem_build_signal():
    with mock.patch("backend.projects.signals.build_filesystem_task.delay") as mock_task:
