...
def test_run_mutation_analysis_task_exception(temp_zip_project):
    ...
    with mock.patch(
        "backend.projects.tasks.run_mutation_analysis", side_effect=Exception("Boom")
    ):
        run_mutation_analysis_task(analysis.id)
...
def test_run_mutation_analysis_task_not_found(caplog):
    ...
    assert (
        "MutationAnalysis 00000000-0000-0000-0000-000000000000 not found" in caplog.text
    )
...
def test_build_filesystem_task_critical_error_project_gone(db):
    ...
    with mock.patch(
        ...
        with mock.patch("backend.projects.tasks.logger.exception") as mock_log:
            with mock.patch(
                "backend.projects.models.Project.objects.get",
                side_effect=[project, Project.DoesNotExist],
            ):
...
def test_run_mutation_analysis_task_critical_error_analysis_gone(db):
    ...
    with mock.patch(
        ...
        with mock.patch("backend.projects.tasks.logger.exception") as mock_log:
            with mock.patch(
                "backend.projects.models.MutationAnalysis.objects.get",
                side_effect=MutationAnalysis.DoesNotExist,
            ):
                ...
                with mock.patch(
                    "backend.projects.models.MutationAnalysis.objects.select_related"
                ) as mock_select:
                    mock_select.return_value.get.side_effect = [
                        analysis,
                        MutationAnalysis.DoesNotExist,
                    ]
                    run_mutation_analysis_task(analysis.id)
